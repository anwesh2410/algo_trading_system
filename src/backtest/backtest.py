...
class Backtest:
    ...
    def monte_carlo_simulation(self, trades_df, symbol, n_simulations=1000, initial_capital=100000):
        """
        ...
        """
        if trades_df.empty:
            logger.warning(f"No trades available for Monte Carlo simulation for {symbol}")
            return None
        ...
        for _ in range(n_simulations):
            # Shuffle returns (randomize trade order)
        ...
        for i in range(min(20, n_simulations)):
            plt.plot(range(len(paths[i])), paths[i], color='blue', alpha=0.1)
            
        # Plot average path
        avg_path = np.mean(np.array(paths), axis=0)
        plt.plot(range(len(avg_path)), avg_path, color='red', linewidth=2, label='Average')
        ...
        # Format the plot
        plt.title(f'{symbol} - Monte Carlo Simulation ({n_simulations} runs)', fontsize=15)
        ...
        plt.tight_layout()
        plt.savefig(self.results_dir / f"{symbol}_monte_carlo.png")
        plt.close()
        
        logger.info(f"Monte Carlo simulation completed for {symbol}")
        logger.info(f"Mean final portfolio value: {mean_final:.2f}, Median final portfolio value: {median_final:.2f}")
        logger.info(f"5th percentile final value: {percentile_5:.2f}, 95th percentile final value: {percentile_95:.2f}")